...
print("DETECCION contador frame = " +str(contador_frame))
...
def definir_borde_superior(y):
	borde_superior = y
	guardar_variables()
	print("borde superior = " + str(borde_superior))

def definir_borde_inferior(y):
	borde_inferior = y
	guardar_variables()
	print("borde inferior = " + str(borde_inferior))

def guardar_variables():

	f = open('config.obj', 'w') 
	pickle.dump([alto_ROI, centro_ROI, borde_superior, borde_inferior], f)
	f.close()
...
def actualizar():
	...
	f = open('config.obj', 'r')
	alto_ROI, centro_ROI, borde_superior, borde_inferior = pickle.load(f)
	f.close()
	...
	cv2.line(img,(0, borde_superior),(width,borde_superior),(255,0,0,200),1	)
	cv2.line(img,(0, borde_inferior),(width,borde_inferior),(0,255,0,200),1	)
...
def correlacion(img, square_wave, template):
	...
	extraer_frame(img, frame_x)



def extraer_frame(img, frame_x):
	...
	frame_detectado = original[centro_frame_y-int(frame_height_px/2):centro_frame_y+int(frame_height_px/2),centro_frame_x-frame_width_px/2:centro_frame_x+int(frame_width_px/2)]
...
cv2.createTrackbar('borde_superior','image',0,int(height/2), definir_borde_superior)
cv2.createTrackbar('borde_inferior','image',int(height/2+1), height, definir_borde_inferior)
cv2.imshow('image',img)
...
while(1):
	...
	if(primera_ejecucion == 1):
	    ...
	    borde_superior = cv2.getTrackbarPos('borde_superior','image')
	    borde_inferior = cv2.getTrackbarPos('borde_inferior','image')
