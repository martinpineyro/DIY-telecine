...
def actualizar():
	...
	img = original.copy()
	ROI = original[centro_ROI-alto_ROI:centro_ROI+alto_ROI,0:width]
	...
	#dibujar una linea al centro de la ROU y un rectangulo delimitandola
...
def ubicar_bordes_film(img, grafica_ROI):
	global borde_superior
	global borde_inferior
	...
	for y in range(height-1, int(height/2), -1):		
		...
		if abs(derivada) > salto:
			borde_inferior = y 
			cv2.line(img,(0,borde_inferior),(width, borde_inferior),(255,0,0),1)
			break
